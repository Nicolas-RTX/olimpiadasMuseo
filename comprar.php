...
// Agrega credenciales
MercadoPago\SDK::setAccessToken('CLAVE DE ACCESO');
...
$preference->back_urls = array(
  "success" => "$dir/entradas.php?pago=aprobado",
  "failure" => "$dir/entradas.php?pago=fallido",
  "pending" => "$dir/entradas.php?pago=pendiente"
);
$preference->auto_return = "approved"; //retorno automatico del usuario

if (isset($_POST['general']) && isset($_POST['fecha']) && isset($_POST['horario'])) {

  $general = $_POST['general'];
  $fecha = $_POST['fecha'];
  $horario = $_POST['horario'];

  if ($general > 0 && $general < 16 && is_numeric($general)) { //Validaciones

    //Creamos el Item con sus propiedades y cantidad
    $item = new MercadoPago\Item();
    $item->title = "Entrada general - $fecha / $horario";
    $item->quantity = $general;
    $item->unit_price = 400;
...
{
  session_start();
  $_SESSION['FLASH']['message'] = "No se pudieron procesar los datos. Vuelva a intentarlo mas tarde";
...
<head>
  ...
  <title>Pagar - Museo de Lanús</title>
</head>
...
<body>
  ...
  <header>
    <section class="entradasHeader">
      <div class="progress">
        <div class="progress-bar progress-bar-striped progress-bar-animated" id="progess-1" bar-full></div>
      </div>
      <div class="entradasHeader__icon" id="icon-1" icon-full><svg class="entradasHeader__svg" id="svg-1" svg-full xmlns="http://www.w3.org/2000/svg" viewBox="0 0 485 485">
          <g>
      ...
      <div class="progress">
        <div class="progress-bar progress-bar-striped progress-bar-animated" id="progess-2"></div>
      </div>
      <div class="entradasHeader__icon" id="icon-2"><svg class="entradasHeader__svg" id="svg-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
          <g>
      ...
      <div class="progress">
        <div class="progress-bar progress-bar-striped progress-bar-animated" id="progess-3"></div>
      </div>
      <div class="entradasHeader__icon" id="icon-3"><svg class="entradasHeader__svg" id="svg-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      ...
      <div class="progress">
        <div class="progress-bar progress-bar-striped progress-bar-animated" id="progess-4"></div>
      </div>
      <div class="entradasHeader__icon" id="icon-4"><svg class="entradasHeader__svg" id="svg-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 80">
          <g>
      ...
      <div class="progress">
        <div class="progress-bar progress-bar-striped progress-bar-animated" id="progess-5"></div>
      </div>
    </section>
  </header>

  <div class="checkout-container">
    <h5>Vas a pagar: Entrada General - Museo de Lanús</h5>
    <h6>Fecha y Horario: <?= $fecha . "/ " . $horario ?></h6>
    <h6>Cantidad: <?= $item->quantity ?></h6>
    <h6>Precio: $<?= $item->unit_price * $item->quantity ?></h6>
    <p class="text-warning text-center">En caso de haber seleccionado mas entradas no estarán <br> incluidas en el pago ya que son gratuitas</p>
    <div class="cho-container"></div>
  </div>
  <script>
    const mp = new MercadoPago('TEST-3aa8fcd5-4c1a-46de-ae5a-b9b336f0ca69', {
      locale: 'es-AR'
    });
